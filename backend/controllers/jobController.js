...
exports.handleJobSeeker = (req, res) => {
  ...
  const profileUrl = req.file.fieldname === 'profile' ? req.file.path : null;

  const seekerData = {
    full_name, email, phone, password, job_title, skills, experience_level, location_preference, profile: profileUrl, resume: resumeUrl,
  };
  insertIntoDatabase('seekers', seekerData, res, req.db);
};
...
exports.handleEmployer = (req, res) => {
  ...
  const employerData = {
    full_name, email, phone, password, company_name, industry, company_size, job_description, logo: logoUrl,
  };
  insertIntoDatabase('employers', employerData, res, req.db);
};
...
exports.applyJob = (req, res) => {
  ...
  const resumeUrl = req.file.path;
  const applicationData = {
    job_id, jobseeker_id: jobSeeker_id, full_name: fullName, email, resume: resumeUrl, cover_letter: coverLetter,
    linkedIn: linkedIn || null, phone: phone || null, portfolio: portfolio || null,
  };
  insertIntoDatabase('job_applications', applicationData, res, req.db);
};

// [Other functions unchanged, included for completeness]
exports.createJobPost = (req, res) => {
  const { Job_title, Company_name, Location, Employment_type, Salary_range, job_description, Key_responsibilities, Requirements, Preferred_qualifications, Benefits, Application_deadline, How_to_apply, employer_id } = req.body;
  if (!Job_title || !Company_name || !Location) {
    return res.status(400).json({ error: 'Job title, company name, and location are required.' });
  }
  const Job_posting_Data = {
    Job_title, Company_name, Location, Employment_type, Salary_range, job_description, Key_responsibilities,
    Requirements, Preferred_qualifications, Benefits, Application_deadline, How_to_apply, employer_id,
  };
  insertIntoDatabase('job_postings', Job_posting_Data, res, req.db);
};
...
exports.getJobListings = (req, res) => {
  ...
  req.db.query(query, queryParams, (err, result) => {
    ...
    result.forEach(job => {
      if (job.company_logo) job.company_logo = job.company_logo;
    });
    let countQuery = `SELECT COUNT(*) as total FROM job_postings jp JOIN employers e ON jp.employer_id = e.employer_id WHERE 1=1`;
    ...
    req.db.query(countQuery, countParams, (err, countResult) => {
      ...
      const totalJobs = countResult[0].total;
      const totalPages = Math.ceil(totalJobs / limit);
      return res.json([result, { totalJobs, totalPages, currentPage: Number(page), jobsPerPage: Number(limit) }]);
    });
  });
};
...
exports.getTotalJobs = (req, res) => {
  ...
  req.db.query(countQuery, (err, result) => {
    ...
    res.json({ total_jobs: result[0].total_jobs });
  });
};
...
exports.getTotalEmployers = (req, res) => {
  ...
  req.db.query(countQuery, (err, result) => {
    ...
    res.json({ total_employers: result[0].total_totalEmployers });
  });
};
...
exports.updateApplicationStatus = (req, res) => {
  ...
  req.db.query(selectQuery, [Job_id, applicant_Id], (selectErr, selectResult) => {
    ...
    const updateQuery = `UPDATE job_applications SET status = ? WHERE job_id = ? AND jobseeker_id = ?`;
    const values = [status, Job_id, applicant_Id];
    req.db.query(updateQuery, values, (err, response) => {
      if (err) {
        console.error('Error updating status:', err);
...
exports.scheduleInterview = (req, res) => {
  ...
  const employer_id = parseInt(req.user.employer_id, 10);
  if (!Job_id || !applicant_Id || !interview_date || !interview_type || isNaN(employer_id)) {
    return res.status(400).json({ error: 'Missing required parameters', details: 'Job_id, applicant_Id, interview_date, and interview_type are required.' });
  }
  const jobIdInt = parseInt(Job_id, 10);
  const applicantIdInt = parseInt(applicant_Id, 10);
  if (isNaN(jobIdInt) || isNaN(applicantIdInt)) {
  ...
  const interviewData = {
    job_id: jobIdInt, jobseeker_id: applicantIdInt, employer_id, interview_date: interviewDateObj, interview_type,
    interview_location: interview_type === 'In-Person' ? interview_location.trim() : null,
    phone_number: interview_type === 'Phone Call' ? phone_number.trim() : null,
    zoom_link: interview_type === 'Video Call' ? zoom_link.trim() : null,
    dress_code: dress_code ? dress_code.trim() : null, document_url: documentUrl, status: 'Scheduled', created_at: new Date(),
  };
  req.db.query('SELECT Job_title, Company_name FROM job_postings WHERE id = ? AND employer_id = ?', [jobIdInt, employer_id], (err, job) => {
    if (err) {
      console.error('Database error:', err);
      return res.status(500).json({ error: 'Database error', details: err.message });
    }
    ...
    const jobDetails = job[0];
    req.db.query('SELECT id FROM interviews WHERE job_id = ? AND jobseeker_id = ? AND status = "Scheduled"', [jobIdInt, applicantIdInt], (err, existingInterview) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ error: 'Database error', details: err.message });
      }
      ...
      req.db.query('INSERT INTO interviews SET ?', interviewData, (err, result) => {
        if (err) {
          console.error('Insert error:', err);
          return res.status(500).json({ error: 'Failed to schedule interview', details: err.message });
        }
        const interviewId = result.insertId;
        req.db.query('UPDATE job_applications SET status = "Interview" WHERE job_id = ? AND jobseeker_id = ?', [jobIdInt, applicantIdInt], (err) => {
          if (err) {
            console.error('Update error:', err);
            return res.status(500).json({ error: 'Failed to update application status', details: err.message });
          }
          const notificationData = {
            interview_id: interviewId, job_id: jobIdInt, interview_date: interviewDateObj.toISOString(), interview_type,
            interview_location: interviewData.interview_location, phone_number: interviewData.phone_number,
            zoom_link: interviewData.zoom_link, dress_code: interviewData.dress_code || 'Not specified',
            document_url: documentUrl ? documentUrl : null, jobTitle: jobDetails.Job_title, companyName: jobDetails.Company_name,
            scheduledAt: interviewData.created_at.toISOString(),
          };
          req.io.to(applicantIdInt.toString()).emit('interviewScheduled', notificationData);
          res.status(201).json({ success: 'Interview scheduled successfully', interview_id: interviewId, interview_date: interviewDateObj.toISOString(), document_url: documentUrl });
        });
      });
    });
  });
...
exports.getJobApplicants = (req, res) => {
  ...
  const query = `
    SELECT 
      ja.job_id, ja.full_name, ja.email, ja.jobseeker_id AS ApplicantId, ja.resume AS applicantResume, ja.cover_letter AS coverLetter,
      ja.linkedin AS applicantLinkedIn, ja.portfolio AS ApplicantPortfolio, ja.created_at AS appliedDate, ja.status,
      s.profile AS ApplicantProfile, i.interview_date AS interviewScheduled
    FROM job_applications ja
    JOIN job_postings jp ON ja.job_id = jp.id
    JOIN seekers s ON ja.jobseeker_id = s.id
    LEFT JOIN interviews i ON ja.job_id = i.job_id AND ja.jobseeker_id = i.jobseeker_id AND i.status = 'Scheduled'
  ...
  req.db.query(query, [Job_id, employerId], (err, result) => {
    ...
    result.forEach(job => {
      if (job.ApplicantProfile) job.ApplicantProfile = job.ApplicantProfile;
      if (job.applicantResume) job.applicantResume = job.applicantResume;
    });
    res.status(200).json(result || []);
  });
};
...
exports.getMyApplication = (req, res) => {
  ...
  const query = `
    SELECT 
      jp.id, jp.Job_title AS title, jp.Company_name AS companyName, jp.Location AS location, jp.Employment_type AS employmentType,
      jp.job_description AS description, jp.salary_range AS salaryRange, jp.created_at AS postedDate, ja.created_at AS appliedDate,
      ja.full_name AS applicantName, ja.email AS applicantEmail, ja.resume AS applicantResume, ja.cover_letter AS coverLetter,
      ja.linkedIn AS applicantLinkedIn, ja.portfolio AS ApplicantPortfolio, ja.status AS applicationStatus,
      i.interview_date AS interviewDate, i.status AS interviewStatus, e.logo AS companyLogo
    FROM job_applications ja
    JOIN job_postings jp ON ja.job_id = jp.id
    JOIN employers e ON jp.employer_id = e.employer_id
    LEFT JOIN interviews i ON ja.job_id = i.job_id AND ja.jobseeker_id = i.jobseeker_id
  ...
  req.db.query(query, [jobSeekerId], (err, result) => {
    if (err) {
      console.error('Database query error:', err);
      return res.status(500).json({ error: "Error fetching applied jobs", details: err.message });
    }
    result.forEach(job => {
      if (job.companyLogo) job.companyLogo = job.companyLogo;
      if (job.applicantResume) job.applicantResume = job.applicantResume;
    });
    res.status(200).json(result);
  });
...
exports.getInterviewCount = (req, res) => {
  ...
  const query = `SELECT COUNT(*) AS interviewCount FROM interviews WHERE jobseeker_id = ? AND status = 'Scheduled'`;
  req.db.query(query, [jobSeeker_id], (err, result) => {
    if (err) {
      console.error('Interview count error:', err);
      return res.status(500).json({ error: 'Database error', details: err.message });
...
exports.getInterviewAlerts = (req, res) => {
  ...
  const query = `
    SELECT i.job_id, i.interview_date, i.interview_type, i.status AS interviewStatus, i.zoom_link, i.dress_code, i.document_url,
           jp.Job_title AS jobTitle, jp.Company_name AS companyName
    FROM interviews i
    JOIN job_postings jp ON i.job_id = jp.id
    WHERE i.jobseeker_id = ? AND i.status = 'Scheduled'
  ...
  req.db.query(query, [jobSeeker_id], (err, result) => {
    ...
    result.forEach((alert) => {
      if (alert.document_url) alert.document_url = alert.document_url;
    });
    res.json(result);
  });
};
...
exports.getInterviews = (req, res) => {
  ...
  const query = `
    SELECT i.id, i.job_id, i.jobseeker_id, i.employer_id, i.interview_date, i.interview_type, i.interview_location,
           i.phone_number, i.zoom_link, i.dress_code, i.document_url, i.status, i.created_at,
           jp.Job_title AS jobTitle, jp.Company_name AS companyName, e.logo AS companyLogo
    FROM interviews i
    JOIN job_postings jp ON i.job_id = jp.id
    JOIN employers e ON i.employer_id = e.employer_id
    WHERE i.jobseeker_id = ? AND i.status IN ('Scheduled', 'Confirmed', 'Declined')
  ...
  req.db.query(query, [jobSeeker_id], (err, results) => {
    ...
    results.forEach((interview) => {
      if (interview.companyLogo) interview.companyLogo = interview.companyLogo;
      if (interview.document_url) interview.document_url = interview.document_url;
    });
    res.status(200).json(results);
  });
};
...
exports.confirmInterview = (req, res) => {
  ...
  const query = `UPDATE interviews SET status = ? WHERE id = ? AND jobseeker_id = ? AND status = 'Scheduled'`;
  req.db.query(query, [response, interview_id, jobSeeker_id], (err, result) => {
    if (err) {
      console.error('Database error:', err);
      return res.status(500).json({ error: 'Database error', details: err.message });
    ...
    req.db.query('SELECT employer_id, job_id FROM interviews WHERE id = ?', [interview_id], (err, interview) => {
      if (err || !interview[0]) {
        console.error('Error fetching interview:', err);
        return res.status(500).json({ error: 'Notification error', details: err?.message });
      }
      const employer_id = interview[0].employer_id;
      const job_id = interview[0].job_id;
      req.db.query('SELECT Job_title, Company_name FROM job_postings WHERE id = ?', [job_id], (err, job) => {
        if (err || !job[0]) {
          console.error('Error fetching job:', err);
          return res.status(500).json({ error: 'Notification error', details: err?.message });
        }
        const notificationData = {
          interview_id, response, jobseeker_id: jobSeeker_id, jobTitle: job[0].Job_title, companyName: job[0].Company_name, timestamp: new Date().toISOString(),
        };
        req.io.to(`employer:${employer_id}`).emit('interviewResponse', notificationData);
        res.json({ success: `${response} successfully` });
      });
...
exports.getEmployerInterviews = (req, res) => {
  ...
  const query = `
    SELECT i.id AS interview_id, i.job_id, i.jobseeker_id AS jobSeekerId, i.interview_date, i.interview_type, i.interview_location,
           i.phone_number, i.zoom_link, i.dress_code, i.document_url, i.status, i.created_at,
           jp.Job_title AS jobTitle, jp.Company_name AS companyName, s.full_name AS jobSeekerName, s.email AS jobSeekerEmail,
           s.phone AS jobSeekerPhone, s.bio AS jobSeekerBio, s.skills AS jobSeekerSkills, s.education AS jobSeekerEducation,
           s.work_experience AS jobSeekerExperience, s.certifications AS jobSeekerCertifications, s.linkedin AS jobSeekerLinkedIn,
           s.github AS jobSeekerGitHub, s.availability AS jobSeekerAvailability, s.profile AS jobSeekerProfilePicture,
           ja.resume AS jobSeekerResume
    FROM interviews i
    JOIN job_postings jp ON i.job_id = jp.id
    JOIN seekers s ON i.jobseeker_id = s.id
  ...
  req.db.query(query, [employer_id], (err, results) => {
    ...
    results.forEach((interview) => {
      if (interview.jobSeekerResume) interview.jobSeekerResume = interview.jobSeekerResume;
      if (interview.document_url) interview.document_url = interview.document_url;
      if (interview.jobSeekerProfilePicture) interview.jobSeekerProfilePicture = interview.jobSeekerProfilePicture;
    });
    res.status(200).json(results);
  });
};
...
exports.getInterviewDetails = (req, res) => {
  ...
  const query = `
    SELECT i.*, jp.Job_title AS jobTitle, jp.Company_name AS companyName, s.full_name AS jobseekerName,
           s.email AS jobseekerEmail, ja.resume AS jobseekerResume
    FROM interviews i
    JOIN job_postings jp ON i.job_id = jp.id
    JOIN seekers s ON i.jobseeker_id = s.id
    JOIN job_applications ja ON i.job_id = ja.job_id AND i.jobseeker_id = ja.jobseeker_id
...
exports.sendMessage = (req, res) => {
  ...
  const senderNameQuery = sender_role === 'seeker' ? 'SELECT full_name AS name FROM seekers WHERE id = ?' : 'SELECT company_name AS name FROM employers WHERE employer_id = ?';
  req.db.query(senderNameQuery, [sender_id], (err, senderResult) => {
    if (err) {
      console.error('Sender query error:', err);
      return res.status(500).json({ error: 'Database error fetching sender', details: err.message });
    ...
    const receiverNameField = receiver_role === 'seeker' ? 'full_name' : 'company_name';
    const userId = receiver_role === 'seeker' ? 'id' : 'employer_id';
    req.db.query(`SELECT ${receiverNameField} AS name FROM ${receiverTable} WHERE ${userId} = ?`, [parsedReceiverId], (err, receiverResult) => {
      if (err) {
        console.error('Receiver query error:', err);
        return res.status(500).json({ error: 'Database error fetching receiver', details: err.message });
      }
      ...
      const messageData = {
        sender_id, sender_role, receiver_id: parsedReceiverId, receiver_role, message: message.trim(), sender_name, created_at: new Date(), is_read: false,
      };
      req.db.query('INSERT INTO messages SET ?', messageData, (err, result) => {
        if (err) {
          console.error('Insert message error:', err);
        ...
        const messageId = result.insertId;
        const notificationData = {
          id: messageId, sender_id, sender_role, sender_name, receiver_id: parsedReceiverId, receiver_role, message: messageData.message,
          created_at: messageData.created_at.toISOString(), is_read: false,
        };
        req.io.to(`${receiver_role}:${parsedReceiverId}`).emit('newMessage', notificationData);
        res.json({ success: 'Message sent successfully', message_id: messageId });
      });
...
exports.getMessages = (req, res) => {
  ...
  console.log("user_id", user_id);
  const query = `
    SELECT m.id, m.sender_id, m.sender_role, m.receiver_id, m.receiver_role, m.message, m.created_at, m.is_read, m.sender_name
    FROM messages m
    WHERE (m.receiver_id = ? AND m.receiver_role = ?) OR (m.sender_id = ? AND m.sender_role = ?)
    ORDER BY m.created_at DESC
  `;
...
exports.markMessageRead = (req, res) => {
  const { message_id } = req.body;
  const user_id = req.user.jobSeeker_id || req.user.employer_id || req.user.id;
  req.db.query('UPDATE messages SET is_read = TRUE WHERE id = ? AND receiver_id = ?', [message_id, user_id], (err, result) => {
    if (err) return res.status(500).json({ error: 'Database error', details: err.message });
    if (result.affectedRows === 0) return res.status(404).json({ error: 'Message not found or unauthorized' });
    res.json({ success: 'Message marked as read' });
  });
...
exports.markAllMessagesRead = (req, res) => {
  const user_id = req.user.jobSeeker_id || req.user.employer_id || req.user.id;
  const user_role = req.user.role;
  const query = `UPDATE messages SET is_read = TRUE WHERE receiver_id = ? AND receiver_role = ? AND is_read = FALSE`;
  req.db.query(query, [user_id, user_role], (err, result) => {
    if (err) {
      console.error('Mark read error:', err);
      return res.status(500).json({ error: 'Database error', details: err.message });
...
exports.getUnreadMessageCount = (req, res) => {
  const user_id = req.user.employer_id || req.user.jobSeeker_id || req.user.id;
  const user_role = req.user.role;
  const query = `SELECT COUNT(*) AS unreadCount FROM messages WHERE receiver_id = ? AND receiver_role = ? AND is_read = FALSE`;
  req.db.query(query, [user_id, user_role], (err, result) => {
    if (err) {
      console.error('Unread count error:', err);
      return res.status(500).json({ error: 'Database error', details: err.message });
...
exports.getEmployerInterviewAlerts = (req, res) => {
  ...
  const query = `
    SELECT i.id AS interview_id, i.job_id, i.jobseeker_id, i.interview_date, i.interview_type, i.status,
           i.zoom_link, i.dress_code, i.document_url, jp.Job_title AS jobTitle, jp.Company_name AS companyName,
           s.full_name AS jobseekerName
    FROM interviews i
    JOIN job_postings jp ON i.job_id = jp.id
    JOIN seekers s ON i.jobseeker_id = s.id
...
exports.getEmployerApplicationAlerts = (req, res) => {
  ...
  const query = `
    SELECT ja.id AS application_id, ja.job_id, ja.jobseeker_id, ja.created_at AS appliedDate, ja.status, ja.full_name AS applicantName,
           jp.Job_title AS jobTitle, jp.Company_name AS companyName
    FROM job_applications ja
    JOIN job_postings jp ON ja.job_id = jp.id
    WHERE jp.employer_id = ? AND ja.status = 'Pending'
...
exports.getEmployerInterviewCount = (req, res) => {
  ...
  const query = `SELECT COUNT(*) AS interviewCount FROM interviews i JOIN job_postings jp ON i.job_id = jp.id WHERE jp.employer_id = ? AND i.status IN ('Scheduled', 'Confirmed')`;
  req.db.query(query, [employer_id], (err, result) => {
    if (err) {
      console.error('Interview count error:', err);
      return res.status(500).json({ error: 'Database error', details: err.message });
...
exports.getEmployerApplicationCount = (req, res) => {
  ...
  const query = `SELECT COUNT(*) AS applicationCount FROM job_applications ja JOIN job_postings jp ON ja.job_id = jp.id WHERE jp.employer_id = ? AND ja.status = 'Pending'`;
  req.db.query(query, [employer_id], (err, result) => {
    if (err) {
      console.error('Application count error:', err);
      return res.status(500).json({ error: 'Database error', details: err.message });
