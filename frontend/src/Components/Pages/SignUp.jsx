...
export const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{6,}$/;
    if (!passwordRegex.test(formData.password)) {
      alert('Password must be at least 6 characters long, with uppercase, lowercase, number, and special character.');
      return;
    }
    ...
    if (role === 'seeker') {
      ...
      formDataToSubmit.append('experience_level', levelMap[formData.experience_level] || 'Entry-level');
      formDataToSubmit.append('location_preference', formData.location_preference || 'remote');
      if (formData.resume) formDataToSubmit.append('resume', formData.resume);
      if (formData.profile) formDataToSubmit.append('profile', formData.profile);
    }

    console.log('FormData:', [...formDataToSubmit.entries()]);
    ...
    try {
      ...
      alert('❌ Error signing up: ' + (err.response?.data?.error || 'Unable to connect.'));
    }
  };

  return (
    <div className="max-w-lg mx-auto px-4 py-8 bg-white">
      <h2 className="text-3xl font-extrabold text-center text-teal-600 mb-8">Job Board Sign-Up</h2>
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label className="block text-base font-medium text-gray-700 mb-2">Select Role</label>
          <div className="flex gap-6">
            <label className="flex items-center text-base text-gray-600">
              <input
                ...
                value="seeker"
                onChange={handleRoleChange}
                className="mr-2 text-teal-500 h-4 w-4"
                required
              />
              Job Seeker
            </label>
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            required
            className="w-full px-4 py-2.5 border border-teal-200 rounded-lg focus:ring-2 focus:ring-teal-400"
          />
        </div>
        <div>
        ...
        {role === 'seeker' && (
          <>
            <div>
              <label className="block text-base font-medium text-gray-700 mb-2">Desired Job Title</label>
              <input
                type="text"
                name="job_title"
                value={formData.job_title}
            ...
            <div>
              <label className="block text-base font-medium text-gray-700 mb-2">Skills</label>
              <input
                type="text"
                name="skills"
                value={formData.skills}
            ...
            <div>
              <label className="block text-base font-medium text-gray-700 mb-2">Experience Level</label>
              <select
                name="experience_level"
                value={formData.experience_level}
                onChange={handleChange}
            ...
            <div>
              <label className="block text-base font-medium text-gray-700 mb-2">Location Preference</label>
              <select
                name="location_preference"
                value={formData.location_preference}
                onChange={handleChange}
            ...
            <div>
              <label className="block text-base font-medium text-gray-700 mb-2">Upload Resume</label>
              <input
                type="file"
                name="resume"
                accept=".pdf,.doc,.docx"
                onChange={handleChange}
                required
                className="w-full px-4 py-2.5 border border-teal-200 rounded-lg"
              />
            </div>
            <div>
              <label className="block text-base font-medium text-gray-700 mb-2">Upload Profile Picture</label>
              <input
                type="file"
                name="profile"
                accept="image/*"
        ...
        <div>
          <label className="flex items-center text-base text-gray-600">
            <input
              ...
              className="mr-2 text-teal-500 h-4 w-4"
            />
            I agree to the <Link to="/terms" className="text-teal-600 font-medium">Terms & Conditions</Link>
          </label>
        </div>
        <button
          type="submit"
          className="w-full px-5 py-2.5 bg-teal-500 text-white font-semibold rounded-lg hover:bg-teal-600"
        >
          Sign Up
        </button>
        <div className="text-center mt-4">
          <p className="text-base text-gray-600">
            Already have an account?{' '}
            <Link to="/login" className="font-semibold text-teal-600">Log in here</Link>
          </p>
        </div>
      </form>
    </div>
