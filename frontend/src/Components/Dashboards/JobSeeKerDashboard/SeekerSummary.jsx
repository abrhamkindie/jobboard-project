...
export const SeekerSummary = ({ onSetActiveContent }) => {
  ...
  const [cardData, setCardData] = useState({
    applicationProgress: { submitted: 0, underReview: 0, awaitingResponse: 0 },
    interviewPipeline: { scheduled: 0, upcoming: 0, completed: 0 },
    jobOffers: { received: 0, pendingDecision: 0, accepted: 0 },
    engagement: { profileViews: 0, employerMessages: 0, savedJobs: 0 },
  });
  ...
  useEffect(() => {
    ...
    const fetchSummary = async () => {
      try {
        const token = localStorage.getItem('token'); // Assumed JWT storage
        ...
        setCardData({
          ...
          interviewPipeline: data.interviewPipeline,
  ...
  const cards = [
    {
      id: 1,
      title: 'Application Progress',
      record1: `Submitted: ${cardData.applicationProgress.submitted}`,
      record2: `Under Review: ${cardData.applicationProgress.underReview}`,
      record3: `Awaiting Response: ${cardData.applicationProgress.awaitingResponse}`,
      onViewDetails: () => handleViewDetails('Applications'),
      highlightColor: 'text-teal-600',
    },
    {
      id: 2,
      title: 'Interview Pipeline',
      record1: `Scheduled: ${cardData.interviewPipeline.scheduled}`,
      record2: `Upcoming: ${cardData.interviewPipeline.upcoming}`,
      record3: `Completed: ${cardData.interviewPipeline.completed}`,
      onViewDetails: () => handleViewDetails('Interviews'),
      highlightColor: 'text-indigo-600',
    },
    {
      id: 3,
      title: 'Job Offers',
      record1: `Received: ${cardData.jobOffers.received}`,
      record2: `Pending Decision: ${cardData.jobOffers.pendingDecision}`,
      record3: `Accepted: ${cardData.jobOffers.accepted}`,
      onViewDetails: () => handleViewDetails('Offers'),
      highlightColor: 'text-green-600',
    },
    {
      id: 4,
      title: 'Engagement',
      record1: `Profile Views: ${cardData.engagement.profileViews}`,
      record2: `Employer Messages: ${cardData.engagement.employerMessages}`,
      record3: `Saved Jobs: ${cardData.engagement.savedJobs}`,
      onViewDetails: () => handleViewDetails('Activity'),
      highlightColor: 'text-orange-600',
    },
  ];
